...
export const usePriceCakeBusd = (): BigNumber => {
  const iccBnbFarm = useFarmFromPid(1)

  const iccPriceBusdAsString = iccBnbFarm.tokenPriceBusd

  const iccPriceBusd = useMemo(() => {
    return new BigNumber(iccPriceBusdAsString)
  }, [iccPriceBusdAsString])
