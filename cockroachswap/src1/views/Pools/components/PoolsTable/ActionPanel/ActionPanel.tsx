...
const ActionPanel: React.FC<ActionPanelProps> = ({ account, pool, userDataLoaded, expanded, breakpoints }) => {
  ...
  const cakeVaultContractAddress = getIccVaultAddress()
  ...
  return (
    <StyledActionPanel expanded={expanded}>
      ...
      <ActionContainer>
        {showSubtitle && (
          <Text mt="4px" mb="16px" color="textSubtle" style={{color:'#BDC2C4'}}>
            {isAutoVault ? t('Automatic restaking') : `${t('Earn')} ICC ${t('Stake').toLocaleLowerCase()} ICC`}
          </Text>
        )}
        {pool.isAutoVault ? (
